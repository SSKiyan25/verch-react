...
import { completeOrderSchema } from "@/features/org/orders/schemas/orgOrderSchemas";
...
export async function completeOrderAction(
  ...
  try {
    ...
    // 3. Validate input
    const validated = completeOrderSchema.parse(input);
    ...
    // 5. Fetch existing invoice
    const { data: invoiceRow, error: invoiceError } = await supabase
      .from("order_invoices")
      .select("id, sequence_number, pdf_path")
    ...
    if (invoiceError || !invoiceRow) {
      return {
        success: false,
        error: "Invoice not found — confirm payment first",
      };
    }
    ...
    const invoiceNumber = buildInvoiceNumber(invoiceRow.sequence_number);

    // 8. Generate final PDF (not draft)
    ...
    // 9. Upload PDF to Firebase — overwrites existing draft
    ...
    console.error("[completeOrderAction]", err);
    const message =
