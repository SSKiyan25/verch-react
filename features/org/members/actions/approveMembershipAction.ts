...
import { approveMembershipSchema } from "@/features/org/members/schemas/membershipSchemas";
...
export async function approveMembershipAction(
  ...
  try {
    // 1. Validate input
    const parsed = approveMembershipSchema.parse(input);
    ...
    // 5. Call RPC
    const { error: rpcError } = await supabase.rpc("approve_membership", {
      p_membership_id: parsed.membershipId,
      p_position: parsed.position ?? null,
    });
    ...
    // Bust the active-members list — newly approved member appears here
    invalidateOrgMembersCache(orgId);
