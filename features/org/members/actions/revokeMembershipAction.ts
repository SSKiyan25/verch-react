...
import { revokeMembershipSchema } from "@/features/org/members/schemas/membershipSchemas";
...
export async function revokeMembershipAction(
  ...
  try {
    // 1. Validate input
    const parsed = revokeMembershipSchema.parse(input);
    ...
    // 5. Call RPC
    const { error: rpcError } = await supabase.rpc("revoke_membership", {
    ...
    // Bust the active-members list — revoked member disappears from here
