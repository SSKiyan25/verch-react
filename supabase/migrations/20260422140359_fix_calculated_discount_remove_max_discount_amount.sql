-- Migration: Fix calculated_discount calculation - remove non-existent max_discount_amount column reference
-- The promotions table does not have a max_discount_amount column, so all COALESCE references to it must be removed
