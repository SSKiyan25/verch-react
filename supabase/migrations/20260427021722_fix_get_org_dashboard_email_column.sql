-- Fix: "column u.email does not exist" error in get_org_dashboard
--
-- The public.users table does not have an email column.
-- Email is stored in auth.users. This fix joins auth.users
-- to retrieve the email for the pending memberships list.
--
-- See: .agent/schemas/users/README.md (users table schema)
...
BEGIN
  ...
  SELECT
    ...
    -- FIXED: Use auth.users (au) instead of public.users (u) for email
