-- ============================================================================
-- Add GCash OCR fields to get_order_detail RPC
-- ============================================================================
-- This migration extends the get_order_detail RPC to return the new OCR
-- verification fields: gcash_ref_no, gcash_amount, ocr_status, ocr_raw_text,
-- ocr_confidence, and ocr_verified_at from the order_payments table.
--
-- These fields are populated by the Firebase Cloud Function after processing
-- the payment screenshot with Google Cloud Vision API.
--
...
-- Recreate with new OCR fields in signature
...
RETURNS TABLE(
  ...
  -- ✅ NEW: GCash OCR fields
  out_gcash_ref_no TEXT,
  out_gcash_amount NUMERIC,
  out_ocr_status TEXT,
  out_ocr_raw_text TEXT,
  out_ocr_confidence NUMERIC,
  out_ocr_verified_at TIMESTAMPTZ,
  out_invoice_id UUID,
...
BEGIN
  ...
  SELECT
    ...
    -- ✅ NEW: GCash OCR fields (cast TEXT columns to TEXT as per RPC authoring rules)
    op.gcash_ref_no::TEXT,
    op.gcash_amount,
    op.ocr_status::TEXT,
    op.ocr_raw_text::TEXT,
    op.ocr_confidence,
    op.ocr_verified_at,
    inv.id,
    CASE
